...
#    apt-get clean; \
#    curl --fail -L https://storage.googleapis.com/golang/go1.17.2.linux-amd64.tar.gz | tar -C /usr/local -xzf - \
FROM quay.io/tnozicka/scylla-golang:1.17 AS builder
WORKDIR /go/src/github.com/scylladb/scylla-operator
COPY . .
RUN make build --warn-undefined-variables

#FROM docker.io/library/ubuntu:20.04
FROM quay.io/tnozicka/scylla-base:latest
#SHELL ["/bin/bash", "-euEo", "pipefail", "-c"]
