...
CRD_PATH ?= pkg/api/scylla/scylla.scylladb.com_scyllaclusters.yaml
...
define generate-crds
	$(call run-crd-gen,$(api_package_dirs),'$(SCYLLA_API_DIR)')

endef

update-crds:
	$(call generate-crds,)
	$(YQ) -e -i '.spec.conversion = {"strategy":"Webhook","webhook":{"conversionReviewVersions":["v1","v2alpha1"],"clientConfig":{"caBundle":"Cg==","service":{"namespace":"scylla-operator","name":"scylla-operator-webhook","path":"/convert"}}}}' ./pkg/api/scylla/scylla.scylladb.com_scyllaclusters.yaml
	$(YQ) -e -i '.metadata.annotations += {"cert-manager.io/inject-ca-from": "scylla-operator/scylla-operator-serving-cert"}' ./pkg/api/scylla/scylla.scylladb.com_scyllaclusters.yaml
