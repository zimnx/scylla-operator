...
type AlertmanagerLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.Alertmanager, err error)
...
// List lists all Alertmanagers in the indexer.
func (s *alertmanagerLister) List(selector labels.Selector) (ret []*v1alpha1.Alertmanager, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.Alertmanager))
	})
	return ret, err
}
...
type AlertmanagerNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.Alertmanager, error)
...
// List lists all Alertmanagers in the indexer for a given namespace.
func (s alertmanagerNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.Alertmanager, err error) {
...
// Get retrieves the Alertmanager from the indexer for a given namespace and name.
func (s alertmanagerNamespaceLister) Get(name string) (*v1alpha1.Alertmanager, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("alertmanager"), name)
	}
	return obj.(*v1alpha1.Alertmanager), nil
}
