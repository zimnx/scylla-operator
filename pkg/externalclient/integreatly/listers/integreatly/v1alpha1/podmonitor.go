...
type PodMonitorLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.PodMonitor, err error)
...
// List lists all PodMonitors in the indexer.
func (s *podMonitorLister) List(selector labels.Selector) (ret []*v1alpha1.PodMonitor, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.PodMonitor))
	})
	return ret, err
}
...
type PodMonitorNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.PodMonitor, error)
...
// List lists all PodMonitors in the indexer for a given namespace.
func (s podMonitorNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.PodMonitor, err error) {
...
// Get retrieves the PodMonitor from the indexer for a given namespace and name.
func (s podMonitorNamespaceLister) Get(name string) (*v1alpha1.PodMonitor, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("podmonitor"), name)
	}
	return obj.(*v1alpha1.PodMonitor), nil
}
