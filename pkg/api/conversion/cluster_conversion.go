...
func convertV1ScyllaClusterToV2alpha1(sc *scyllav1.ScyllaCluster) (*scyllav2alpha1.ScyllaCluster, error) {
	...
	status, err := convertV1ScyllaClusterStatusToV2alpha1(sc.Spec, sc.Status)
	if err != nil {
	...
	resourcemerge.MergeMapInPlaceWithoutRemovalKeys(&sc.Annotations, annotations)
...
func convertV1ScyllaClusterStatusToV2alpha1(scSpec scyllav1.ScyllaClusterSpec, scStatus scyllav1.ScyllaClusterStatus) (*scyllav2alpha1.ScyllaClusterStatus, error) {
	var nodes, updatedNodes, readyNodes int32
	var datacenters []scyllav2alpha1.DatacenterStatus

	if len(scStatus.Racks) > 0 {
		var upgradeStatus *scyllav2alpha1.UpgradeStatus
		if scStatus.Upgrade != nil {
			upgradeStatus = &scyllav2alpha1.UpgradeStatus{
				State:             scStatus.Upgrade.State,
				FromVersion:       scStatus.Upgrade.FromVersion,
				ToVersion:         scStatus.Upgrade.ToVersion,
				SystemSnapshotTag: scStatus.Upgrade.SystemSnapshotTag,
				DataSnapshotTag:   scStatus.Upgrade.DataSnapshotTag,
			}
		}
		...
		for _, rack := range scSpec.Datacenter.Racks {
			rackStatus, ok := scStatus.Racks[rack.Name]
			if !ok {
				continue
			}
			...
			if rackStatus.Stale != nil {
				stale = pointer.Bool(*rackStatus.Stale)
			}
			...
			rackStatuses = append(rackStatuses, scyllav2alpha1.RackStatus{
				Name:                    rack.Name,
				UpdatedVersion:          fmt.Sprintf("%s:%s", scSpec.Repository, rackStatus.Version),
				CurrentVersion:          fmt.Sprintf("%s:%s", scSpec.Repository, rackStatus.Version),
		...
		currentVersion := scStatus.Racks[scSpec.Datacenter.Racks[len(scSpec.Datacenter.Racks)-1].Name].Version
		updatedVersion := scStatus.Racks[scSpec.Datacenter.Racks[0].Name].Version

		datacenters = append(datacenters, scyllav2alpha1.DatacenterStatus{
			Name:           scSpec.Datacenter.Name,
			CurrentVersion: fmt.Sprintf("%s:%s", scSpec.Repository, currentVersion),
			UpdatedVersion: fmt.Sprintf("%s:%s", scSpec.Repository, updatedVersion),
	...
	return &scyllav2alpha1.ScyllaClusterStatus{
		ObservedGeneration: scStatus.ObservedGeneration,
...
func convertV2alpha1ScyllaClusterStatusToV1(sc *scyllav2alpha1.ScyllaCluster) (*scyllav1.ScyllaClusterStatus, error) {
		...
		for _, dcStatus := range sc.Status.Datacenters {
			...
			for i, rackStatus := range dcStatus.Racks {
				...
				for _, rackCondition := range rackStatus.Conditions {
					...
					default:
						return nil, fmt.Errorf("unknown rack condition: %q", rackCondition)
					}
	...
	if v, ok := sc.Annotations[naming.ScyllaClusterV1Annotation]; ok {
		scv1 = &scyllav1.ScyllaCluster{}
		if err := json.NewDecoder(bytes.NewBufferString(v)).Decode(scv1); err != nil {
			return nil, fmt.Errorf("can't decode v1.ScyllaCluster from annotation: %w", err)
		}
	}
...
func convertV2alpha1ScyllaClusterToV1(sc *scyllav2alpha1.ScyllaCluster) (*scyllav1.ScyllaCluster, error) {
	...
	status, err := convertV2alpha1ScyllaClusterStatusToV1(sc)
	if err != nil {
	...
	resourcemerge.MergeMapInPlaceWithoutRemovalKeys(&sc.Annotations, customAnnotations)
