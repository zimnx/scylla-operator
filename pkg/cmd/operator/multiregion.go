...
func (o *MultiRegionOperatorOptions) run(ctx context.Context, streams genericclioptions.IOStreams) error {
	...
	rkccc, err := remotekubeclusterconfig.NewController(
		...
		[]multiregiondynamicinformers.DynamicRegion{o.remoteDynamicClient},
	)
	if err != nil {
		return err
	}
	...
	scc, err := scyllacluster.NewController(
		...
		remoteDynamicInformer.ForResource(v1alpha1.GroupVersion.WithResource("scylladatacenters"), v1alpha1.GroupVersion.WithKind("ScyllaDatacenter")),
		remoteDynamicInformer.ForResource(corev1.SchemeGroupVersion.WithResource("namespaces"), corev1.SchemeGroupVersion.WithKind("Namespace")),
	)
	if err != nil {
		return err
	}
