...
func NewGenGitReleaseNotesCommand(ctx context.Context, streams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			pullRequests, err := pullRequestsFromMergeCommits(ctx, ghClient, o)
			if err != nil {
				return err
			}

			if err := renderReleaseNotes(o.Out, o.Release, pullRequests); err != nil {
				return err
			}
...
func pullRequestsFromMergeCommits(ctx context.Context, ghClient *github.Client, o *GenerateOptions) ([]*github.PullRequest, error) {
	repo, err := git.PlainOpen(o.RepositoryPath)
	if err != nil {
		return nil, err
	}

	commits, err := listCommitsBetween(repo, o.TagFrom, o.TagTo)
	if err != nil {
		return nil, err
	}
	...
	for _, c := range commits {
		...
		pr, _, err := ghClient.PullRequests.Get(ctx, o.OrganizationName, o.RepositoryName, n)
		if err != nil {
			return nil, err
		}
