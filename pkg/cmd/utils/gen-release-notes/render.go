...
func renderReleaseNotes(out io.Writer, release string, pullRequests []*github.PullRequest) error {
...
func formatPR(pr *github.PullRequest) string {
	details := fmt.Sprintf("[#%d](%s),[@%s](%s)", pr.GetNumber(), pr.GetHTMLURL(), pr.GetUser().GetLogin(), pr.GetUser().GetHTMLURL())
	return fmt.Sprintf("* %s (%s)", pr.GetTitle(), details)
}
