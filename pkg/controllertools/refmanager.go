...
func (m *ControllerRefManager[T]) canAdopt() error {
	...
	if fresh.GetUID() != m.Controller.GetUID() {
		// TODO: add GVK
		return fmt.Errorf("original controller %v/%v is gone: got uid %v, wanted %v", m.Controller.GetNamespace(), m.Controller.GetName(), fresh.GetUID(), m.Controller.GetUID())
	}

	if fresh.GetDeletionTimestamp() != nil {
		// TODO: add GVK
		return fmt.Errorf("%v/%v has just been deleted at %v", m.Controller.GetNamespace(), m.Controller.GetName(), m.Controller.GetDeletionTimestamp())
	}
...
func (m *ControllerRefManager[T]) AdoptObject(obj T) error {
	...
	if err != nil {
		...
		return fmt.Errorf("can't adopt %s %v/%v (%v): %v", gvk, obj.GetNamespace(), obj.GetName(), obj.GetUID(), err)
	}
