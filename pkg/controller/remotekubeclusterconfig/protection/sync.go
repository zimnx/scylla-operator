...
func (rkccpc *Controller) addFinalizer(ctx context.Context, rkcc *scyllav1alpha1.RemoteKubeClusterConfig) error {
	rkccCopy := rkcc.DeepCopy()
	rkccCopy.ObjectMeta.Finalizers = append(rkccCopy.ObjectMeta.Finalizers, naming.RemoteKubeClusterConfigFinalizer)

	_, err := rkccpc.scyllaClient.ScyllaV1alpha1().RemoteKubeClusterConfigs().Update(ctx, rkccCopy, metav1.UpdateOptions{})
	if err != nil {
...
func (rkccpc *Controller) removeFinalizer(ctx context.Context, rkcc *scyllav1alpha1.RemoteKubeClusterConfig) error {
	rkccCopy := rkcc.DeepCopy()
	rkccCopy.ObjectMeta.Finalizers = slices.RemoveString(naming.RemoteKubeClusterConfigFinalizer, rkccCopy.ObjectMeta.Finalizers)
