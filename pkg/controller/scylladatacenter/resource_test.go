...
func TestStatefulSetForRack(t *testing.T) {
	newBasicRack := func() scyllav1alpha1.RackSpec {
		return scyllav1alpha1.RackSpec{
			...
			Scylla: &scyllav1alpha1.ScyllaOverrides{
				CustomConfigMapRef: &corev1.LocalObjectReference{
					Name: "scylla-config",
				},
			...
			ScyllaManagerAgent: &scyllav1alpha1.ScyllaManagerAgentOverrides{
				Resources: &corev1.ResourceRequirements{
					Requests: map[corev1.ResourceName]resource.Quantity{
				...
				CustomConfigSecretRef: &corev1.LocalObjectReference{
		...
		return &scyllav1alpha1.ScyllaDatacenter{
			...
			Spec: scyllav1alpha1.ScyllaDatacenterSpec{
				...
				ScyllaManagerAgent: &scyllav1alpha1.ScyllaManagerAgent{
					Image: "docker.io/scylladb/scylla-manager-agent:latest",
				},
				DatacenterName: "dc",
	...
	newBasicStatefulSetLabelsWithVersion := func() map[string]string {
		...
		m["scylla/scylla-version"] = "latest"
		return m
	}
						...
						Containers: []corev1.Container{
							{
								...
								Resources: *newBasicRack().Scylla.Resources,
								VolumeMounts: []corev1.VolumeMount{
									{
										Name:      "data",
							...
							},
							{
								...
								Resources: *newBasicRack().ScyllaManagerAgent.Resources,
							},
						},
				...
				VolumeClaimTemplates: []corev1.PersistentVolumeClaim{
					{
						...
						Spec: corev1.PersistentVolumeClaimSpec{
							...
							StorageClassName: newBasicRack().Scylla.Storage.StorageClassName,
							Resources:        newBasicRack().Scylla.Storage.Resources,
						},
					},
				},
