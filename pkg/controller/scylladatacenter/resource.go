...
func applyScyllaClusterV1Annotations(sd *scyllav1alpha1.ScyllaDatacenter, sts *appsv1.StatefulSet, r scyllav1alpha1.RackSpec, sidecarImage string) error {
	...
	// var v1Rack *scyllav1.RackSpec
	// for _, rack := range scv1.Spec.Datacenter.Racks {
	// 	if r.Name == rack.Name {
	// 		v1Rack = rack.DeepCopy()
	// 		break
	// 	}
	// }
	//
	// if v1Rack == nil {
	// 	return fmt.Errorf("can't find rack %q from v1.ScyllaCluster annotation", r.Name)
	// }

	// sts.Spec.Replicas = pointer.Int32(v1Rack.Members)
	// sts.Spec.VolumeClaimTemplates[0].Spec.StorageClassName = v1Rack.Storage.StorageClassName
	// sts.Spec.VolumeClaimTemplates[0].Spec.Resources = corev1.ResourceRequirements{
	// 	Requests: corev1.ResourceList{
	// 		corev1.ResourceStorage: resource.MustParse(v1Rack.Storage.Capacity),
	// 	},
	// }
	// if v1Rack.Placement != nil {
	// 	sts.Spec.Template.Spec.Tolerations = v1Rack.Placement.Tolerations
	// 	sts.Spec.Template.Spec.Affinity = &corev1.Affinity{
	// 		NodeAffinity:    v1Rack.Placement.NodeAffinity,
	// 		PodAffinity:     v1Rack.Placement.PodAffinity,
	// 		PodAntiAffinity: v1Rack.Placement.PodAntiAffinity,
	// 	}
	// }

	// for _, volume := range v1Rack.Volumes {
	// 	sts.Spec.Template.Spec.Volumes = append(sts.Spec.Template.Spec.Volumes, volume)
	// }
	//
	// scyllaContainerIdx, err := naming.FindScyllaContainer(sts.Spec.Template.Spec.Containers)
	// if err != nil {
	// 	return fmt.Errorf("can't find scylla container in statefulset: %w", err)
	// }
	//
	// sts.Spec.Template.Spec.Containers[scyllaContainerIdx].Resources = v1Rack.Resources
	// for _, volumeMount := range v1Rack.VolumeMounts {
	// 	sts.Spec.Template.Spec.Containers[scyllaContainerIdx].VolumeMounts = append(sts.Spec.Template.Spec.Containers[scyllaContainerIdx].VolumeMounts, volumeMount)
	// }
	//
	// if len(v1Rack.ScyllaConfig) > 0 {
	// 	volumeFound := false
	// 	for i, volume := range sts.Spec.Template.Spec.Volumes {
	// 		if volume.Name == scyllaConfigVolumeName {
	// 			volumeFound = true
	//
	// 			sts.Spec.Template.Spec.Volumes[i].ConfigMap.LocalObjectReference = corev1.LocalObjectReference{Name: v1Rack.ScyllaConfig}
	// 			break
	// 		}
	// 	}
	//
	...
	// }
	//
	// agentContainerIdx, err := naming.FindContainerWithName(sts.Spec.Template.Spec.Containers, naming.ScyllaManagerAgentContainerName)
	// if err != nil {
	// 	image := fmt.Sprintf("%s:%s", scv1.Spec.AgentRepository, scv1.Spec.AgentVersion)
	// 	var agentCustomSecretRef *corev1.LocalObjectReference
	// 	if len(v1Rack.ScyllaAgentConfig) != 0 {
	// 		agentCustomSecretRef = &corev1.LocalObjectReference{Name: v1Rack.ScyllaAgentConfig}
	// 	}
	// 	cnt, volumes := agentContainer(image, &v1Rack.AgentResources, agentCustomSecretRef, v1Rack.AgentVolumeMounts)
	// 	sts.Spec.Template.Spec.Containers = append(sts.Spec.Template.Spec.Containers, cnt)
	// 	sts.Spec.Template.Spec.Volumes = append(sts.Spec.Template.Spec.Volumes, volumes...)
	// } else {
	// 	sts.Spec.Template.Spec.Containers[agentContainerIdx].Resources = v1Rack.AgentResources
	//
	// 	if len(v1Rack.ScyllaAgentConfig) != 0 {
	// 		volumeFound := false
	// 		for i, volume := range sts.Spec.Template.Spec.Volumes {
	// 			if volume.Name == scyllaAgentConfigVolumeName {
	// 				volumeFound = true
	//
	// 				sts.Spec.Template.Spec.Volumes[i].Secret = &corev1.SecretVolumeSource{
	// 					SecretName: v1Rack.ScyllaAgentConfig,
	// 				}
	// 				break
	// 			}
	// 		}
	...
	// 				},
	// 			})
	// 			sts.Spec.Template.Spec.Containers[agentContainerIdx].VolumeMounts = append(sts.Spec.Template.Spec.Containers[agentContainerIdx].VolumeMounts,
	// 				corev1.VolumeMount{
	// 					Name:      scyllaAgentConfigVolumeName,
	// 					MountPath: path.Join(naming.ScyllaAgentConfigDirName, naming.ScyllaAgentConfigFileName),
	// 					SubPath:   naming.ScyllaAgentConfigFileName,
	...
	// 				})
	//
	// 			sts.Spec.Template.Spec.Containers[agentContainerIdx].Args = append(sts.Spec.Template.Spec.Containers[agentContainerIdx].Args,
	// 				"-c", path.Join(naming.ScyllaAgentConfigDirName, naming.ScyllaAgentConfigFileName))
	// 		}
	// 	}
	//
	// 	for _, volumeMount := range v1Rack.AgentVolumeMounts {
	// 		sts.Spec.Template.Spec.Containers[agentContainerIdx].VolumeMounts = append(sts.Spec.Template.Spec.Containers[agentContainerIdx].VolumeMounts, volumeMount)
	// 	}
	// }
