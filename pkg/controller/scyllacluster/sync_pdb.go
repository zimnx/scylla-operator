...
func (sdc *Controller) syncPodDisruptionBudgets(
	...
	for _, pdb := range pdbs {
		...
		err = sdc.kubeClient.PolicyV1beta1().PodDisruptionBudgets(pdb.Namespace).Delete(ctx, pdb.Name, metav1.DeleteOptions{
	...
	_, _, err = resourceapply.ApplyPodDisruptionBudget(ctx, sdc.kubeClient.PolicyV1beta1(), sdc.pdbLister, sdc.eventRecorder, requiredPDB, true)
	if err != nil {
