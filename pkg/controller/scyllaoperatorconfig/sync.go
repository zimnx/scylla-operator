...
func (opc *Controller) sync(ctx context.Context) error {
	soc, err := opc.scyllaOperatorConfigLister.Get(globalConfigName)
	if err != nil {
		if !apierrors.IsNotFound(err) {
			return fmt.Errorf("can't get sdcyllaoperatorconfig %q: %w", globalConfigName, err)
		}
		...
		if err != nil {
			return fmt.Errorf("can't create scyllaoperatorconfig %q: %w", globalConfigName, err)
		}
