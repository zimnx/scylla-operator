...
func TestGetPartitionUUID(t *testing.T) {
	...
	tt := []struct {
	...
	}{
		{
			...
			expectedError:         fmt.Errorf(`can't get block device "/dev/md0": %w`, fmt.Errorf(`can't list block device "/dev/md0": %w`, fmt.Errorf(`failed to run lsblk with args: [--json --nodeps --paths --fs --output=NAME,MODEL,FSTYPE,PARTUUID /dev/md0]: %w, stderr: "some random error"`, testingexec.FakeExitError{Status: 666}))),
		},
		{
...
func TestIsBlockDevice(t *testing.T) {
	...
	tt := []struct {
	...
	}{
		{
			...
			expectedError:       fmt.Errorf(`failed to run lsblk with args: /dev/md0: %w, stderr: "some random error"`, testingexec.FakeExitError{Status: 666}),
		},
		{
			...
			commands: []exectest.Command{
				{
					Cmd:    "lsblk",
					Args:   []string{"--json", "--nodeps", "--paths", "--fs", "--output=NAME,MODEL,FSTYPE,PARTUUID", "/dev/md0"},
					Stderr: []byte("lsblk: /dev/md0: not a block device"),
					Err:    testingexec.FakeExitError{Status: 32},
				},
			},
			expectedDevice: nil,
			expectedError:  fmt.Errorf(`can't list block device "/dev/md0": %w`, fmt.Errorf(`failed to run lsblk with args: [--json --nodeps --paths --fs --output=NAME,MODEL,FSTYPE,PARTUUID /dev/md0]: %w, stderr: "lsblk: /dev/md0: not a block device"`, testingexec.FakeExitError{Status: 32})),
		},
		{
...
func TestListBlockDevices(t *testing.T) {
	...
	tt := []struct {
		...
		},
		{
			...
			expectedDevices: nil,
			expectedError:   fmt.Errorf(`failed to run lsblk with args: [--json --nodeps --paths --fs --output=NAME,MODEL,FSTYPE,PARTUUID]: %w, stderr: "some random error"`, testingexec.FakeExitError{Status: 666}),
		},
	}
