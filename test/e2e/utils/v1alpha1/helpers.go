...
func IsScyllaDatacenterRolledOut(sd *scyllav1alpha1.ScyllaDatacenter) (bool, error) {
	...
	for _, r := range sd.Spec.Racks {
		...
		if rackStatus.Nodes == nil {
			return true, fmt.Errorf("nodes shouldn't be nil")
		}

		if rackStatus.Nodes != r.Nodes {
			return false, nil
		}

		if rackStatus.ReadyNodes == nil {
			return true, fmt.Errorf("readyNodes shouldn't be nil")
		}

		if rackStatus.ReadyNodes != r.Nodes {
			return false, nil
		}

		if rackStatus.UpdatedNodes == nil {
			return true, fmt.Errorf("updatedNodes shouldn't be nil")
		}

		if *rackStatus.UpdatedNodes != *r.Nodes {
			return false, nil
		}

		if rackStatus.Image != sd.Spec.Scylla.Image {
			return false, nil
		}
	}
...
func WaitForScyllaDatacenterState(ctx context.Context, client scyllav1alpha1client.ScyllaV1alpha1Interface, namespace string, name string, options WaitForStateOptions, condition func(sd *scyllav1alpha1.ScyllaDatacenter) (bool, error), additionalConditions ...func(datacenter *scyllav1alpha1.ScyllaDatacenter) (bool, error)) (*scyllav1alpha1.ScyllaDatacenter, error) {
	return WaitForObjectState[*scyllav1alpha1.ScyllaDatacenter, *scyllav1alpha1.ScyllaDatacenterList](ctx, client.ScyllaDatacenters(namespace), name, options, condition, additionalConditions...)
}
...
func GetNodeName(sd *scyllav1alpha1.ScyllaDatacenter, idx int) string {
	return fmt.Sprintf(
		"%s-%s-%s-%d",
		sd.Name,
		sd.Spec.DatacenterName,
		sd.Spec.Racks[0].Name,
		idx,
	)
}
