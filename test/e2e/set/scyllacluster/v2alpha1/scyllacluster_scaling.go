...
var _ = g.Describe("ScyllaCluster scaling", func() {
	...
	g.DescribeTable("should support scaling",
		func(makeScyllaCluster func() (*v2alpha1.ScyllaCluster, func())) {
			...
			sc, err = v2alpha1utils.WaitForScyllaClusterState(waitCtx1, f.ScyllaClient().ScyllaV2alpha1(), sc.Namespace, sc.Name, v2alpha1utils.WaitForStateOptions{}, v2alpha1utils.IsScyllaClusterRolledOut)
			o.Expect(err).NotTo(o.HaveOccurred())

			// TODO: passing ScyllaAdminClient because e2e-user doesn't have permission in remoteNamespace
			verifyScyllaCluster(ctx, f.ScyllaAdminClient(), sc)
			...
			sc, err = v2alpha1utils.WaitForScyllaClusterState(waitCtx2, f.ScyllaClient().ScyllaV2alpha1(), sc.Namespace, sc.Name, v2alpha1utils.WaitForStateOptions{}, v2alpha1utils.IsScyllaClusterRolledOut)
			o.Expect(err).NotTo(o.HaveOccurred())
		...
		g.Entry("remote cluster", func() (*v2alpha1.ScyllaCluster, func()) {
			...
			sa := newServiceAccountToSelf(ctx, f.KubeAdminClient(), f.Namespace())
...
func newServiceAccountToSelf(ctx context.Context, adminClient kubernetes.Interface, namespace string) *corev1.ServiceAccount {
	...
	clusterRole, err := adminClient.RbacV1().ClusterRoles().Create(ctx, &rbacv1.ClusterRole{
		ObjectMeta: metav1.ObjectMeta{
			Name: userSA.Name,
		},
		Rules: []rbacv1.PolicyRule{
			{
				Verbs:     []string{"create", "patch", "update", "delete", "deletecollection", "get", "list", "watch"},
				APIGroups: []string{""},
			...
			},
			{
				...
				Resources: []string{"scylladatacenters"},
			},
			{
				Verbs:     []string{"get", "list", "watch"},
				APIGroups: []string{"scylla.scylladb.com"},
				Resources: []string{"scylladatacenters/status"},
			},
		},
	}, metav1.CreateOptions{})
